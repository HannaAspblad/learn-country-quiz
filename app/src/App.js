...
const StartPage = (props) => {
  ...
  const validFeature = () => {
    if (!loadingAlt) {
      const profileFeature = snapshotAlt.val();
      if (
        profileFeature.numQuestions ==
  ...
  return (
    <div className="page">
      ...
      {validFeature(localStorage.getItem("profile"), "NumQuestions") &&
      JSON.parse(localStorage.getItem("improvedQuestions")) ? (
        <div>
        ...
        <p>
          You need to switch on Generated questions in Setup to be able to
...
const GamePage = ({ gameId, playerId, profile }) => {
  ...
  if (game && game.status === "playing")
    return (
      <QuestionPage gameId={gameId} playerId={playerId} profile={profile} />
    );
        ...
        {Object.entries(question.alternatives).map(([k, countryCode]) => {
          ...
          return (
            <div
              ...
              } ${correct && "alt-green"} ${correct === false && "alt-red"}`}
              key={countryCode}
              title={countryCode}
  ...
  return (
    <div className="cookie-page-wrapper">
      ...
      <p>
        ...
        owners of the site. The table below explains the cookies we use and why.
      </p>
      <div className="cookie-grid-container">
        ...
        <div className="cookie-grid-item">
          <p>
            ...
            Flag Game. Where users have previously indicated a preference, that
